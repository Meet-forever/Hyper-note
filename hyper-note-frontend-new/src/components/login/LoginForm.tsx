    ...
    return (
        <div className="mx-auto max-w-xs w-full pb-8 pt-8 px-8 bg-opacity-70 rounded-xl z-10 bg-gray-100 shadow-lg ">
            <form className='flex flex-col gap-y-2' onSubmit={handleSubmit}>
                <input onChange={(e) => setLogin(i => ({ ...i, email: e.target.value }))} required={true} name="email" type="email" className={`px-4 py-3 w-full text-md rounded-md focus:outline-none focus:shadow-outline hover:bg-gray-100 ${error.email ? `border border-red-500` : ``}`} placeholder="Email" />
                {error.email !== "" ? <p className="text-red-500 px-2 text-xs italic">{error.email}</p> : <p className="px-2 w-full"> </p>}
                <input onChange={(e) => setLogin(i => ({ ...i, password: e.target.value }))} required={true} name="password" type="password" className={`px-4 py-3 w-full text-md rounded-md focus:outline-none focus:shadow-outline hover:bg-gray-100 ${error.password ? `border border-red-500` : ``} `} placeholder="Password" />
